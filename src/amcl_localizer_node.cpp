...
inline Eigen::Matrix4f Pose2DToMatrix4f(const Pose2D &pose) {
...
public:
  ...
  PointCloudT::Ptr makeOccupiedPointCloud() const {
    PointCloudT::Ptr cloud(new PointCloudT);
    if (!ready_ || occupied_cells_ == 0 || res_ <= 0.0) {
      return cloud;
    }

    cloud->reserve(occupied_cells_);
    const double c = std::cos(oyaw_);
    const double s = std::sin(oyaw_);
    for (int y = 0; y < h_; ++y) {
      for (int x = 0; x < w_; ++x) {
        const std::size_t id = idx(x, y);
        ...
        const double lx = (static_cast<double>(x) + 0.5) * res_;
        const double ly = (static_cast<double>(y) + 0.5) * res_;
        PointT pt;
        pt.x = static_cast<float>(ox_ + c * lx - s * ly);
        pt.y = static_cast<float>(oy_ + s * lx + c * ly);
        pt.z = 0.0f;
        pt.intensity = 1.0f;
        cloud->push_back(pt);
      }
...
private:
  ...
  void loadMapParamsYaml_() {
    ...
    LoadFieldFromYaml(y, "viz_publish_scan_cloud", viz_publish_scan_cloud_);
    LoadFieldFromYaml(y, "viz_scan_cloud_topic", viz_scan_cloud_topic_);
    ...
    LoadFieldFromYaml(y, "dyn_filter_enable", dyn_cfg_.enable);
    LoadFieldFromYaml(y, "dyn_grid_res_m", dyn_cfg_.grid_res_m);
    LoadFieldFromYaml(y, "dyn_inflate_m", dyn_cfg_.inflate_m);
    LoadFieldFromYaml(y, "dyn_padding_m", dyn_cfg_.padding_m);
    LoadFieldFromYaml(y, "dyn_keep_ratio", dyn_cfg_.keep_ratio);
    LoadFieldFromYaml(y, "dyn_min_keep_points", dyn_cfg_.min_keep_points);
    LoadFieldFromYaml(y, "dyn_hard_max_dist_m", dyn_cfg_.hard_max_dist_m);
    LoadFieldFromYaml(y, "dyn_ray_enable", dyn_cfg_.ray_enable);
    LoadFieldFromYaml(y, "dyn_ray_margin_near_m", dyn_cfg_.ray_margin_near_m);
    LoadFieldFromYaml(y, "dyn_ray_max_range_m", dyn_cfg_.ray_max_range_m);
    LoadFieldFromYaml(y, "dyn_ray_hit_eps_m", dyn_cfg_.ray_hit_eps_m);
    LoadFieldFromYaml(y, "dyn_ray_max_steps", dyn_cfg_.ray_max_steps);
    LoadFieldFromYaml(y, "dyn_ray_confirm_df_m", dyn_cfg_.ray_confirm_df_m);
    LoadFieldFromYaml(y, "dyn_ray_step_m", dyn_cfg_.ray_step_m);
    LoadFieldFromYaml(y, "dyn_front_protect_enable",
                      dyn_cfg_.front_protect_enable);
    LoadFieldFromYaml(y, "dyn_front_protect_x_min_m",
                      dyn_cfg_.front_protect_x_min_m);
    LoadFieldFromYaml(y, "dyn_front_protect_x_max_m",
                      dyn_cfg_.front_protect_x_max_m);
    LoadFieldFromYaml(y, "dyn_front_protect_abs_y_max_m",
                      dyn_cfg_.front_protect_abs_y_max_m);
    LoadFieldFromYaml(y, "dyn_front_protect_use_y_axis",
                      dyn_cfg_.front_protect_use_y_axis);
    LoadFieldFromYaml(y, "dyn_front_protect_forward_sign",
                      dyn_cfg_.front_protect_forward_sign);
    LoadFieldFromYaml(y, "dyn_front_protect_max_drop_ratio",
                      dyn_cfg_.front_protect_max_drop_ratio);
    LoadFieldFromYaml(y, "dyn_front_protect_min_in_points",
  ...
  void beginRelocRefine_(const Pose2D &seed_pose, const Pose2D &odom_pose,
    ...
    reloc_refine_.xy_range_m = 1.0;
    reloc_refine_.yaw_range_rad = Deg2Rad(18.0);
    reloc_refine_.frames_left = 12;
  ...
  bool stepRelocRefine_(const std::vector<Eigen::Vector2f> &scan_xy_base,
    ...
    if (reloc_refine_.frames_left <= 6) {
      opt.yaw_step_rad = Deg2Rad(0.25);
    }
    ...
    reloc_refine_.xy_range_m = std::max(0.12, reloc_refine_.xy_range_m * 0.80);
    reloc_refine_.yaw_range_rad =
        std::max(Deg2Rad(2.0), reloc_refine_.yaw_range_rad * 0.80);
  ...
  double manualRelocCombinedScore_(const LocalRelocResult &r) const {
    return manualRelocCombinedScore_(r, 0.10);
  }

  double manualRelocCombinedScore_(const LocalRelocResult &r,
                                   double xy_prior_weight) const {
    ...
    return localRelocBaseScore_(r.best_score, r.second_score, r.valid_fraction,
                                0.4, 0.2) -
           xy_prior_weight * HuberUnit(dxy / xy_ref);
  }
  ...
  double refineCombinedScore_(const LocalRelocResult &r,
    ...
    return localRelocBaseScore_(r.best_score, r.second_score, r.valid_fraction,
                                0.4, 0.2) -
           0.15 * HuberUnit(dxy / 0.12) -
           0.02 * HuberUnit(dyaw / Deg2Rad(2.0));
  }
  ...
  double relocRefineBasinScore_(const RelocRefineBasin &b) const {
    ...
    double score = 0.35 * b.best_combined_score + 0.25 * avg_combined +
                   0.40 * std::min(1.0, static_cast<double>(b.hits) / 5.0);
    if (b.hits <= 1) {
      score -= 0.08;
    } else if (b.hits == 2) {
      score -= 0.03;
  ...
  void loadParams() {
    ...
    pnh_.param("viz_publish_scan_cloud", viz_publish_scan_cloud_,
               viz_publish_scan_cloud_);
    pnh_.param("viz_scan_cloud_topic", viz_scan_cloud_topic_,
               viz_scan_cloud_topic_);
    ...
    pnh_.param("dyn_filter_enable", dyn_cfg_.enable, dyn_cfg_.enable);
    pnh_.param("dyn_grid_res_m", dyn_cfg_.grid_res_m, dyn_cfg_.grid_res_m);
    pnh_.param("dyn_inflate_m", dyn_cfg_.inflate_m, dyn_cfg_.inflate_m);
    pnh_.param("dyn_padding_m", dyn_cfg_.padding_m, dyn_cfg_.padding_m);
    pnh_.param("dyn_keep_ratio", dyn_cfg_.keep_ratio, dyn_cfg_.keep_ratio);
    pnh_.param("dyn_min_keep_points", dyn_cfg_.min_keep_points,
               dyn_cfg_.min_keep_points);
    pnh_.param("dyn_hard_max_dist_m", dyn_cfg_.hard_max_dist_m,
               dyn_cfg_.hard_max_dist_m);
    pnh_.param("dyn_ray_enable", dyn_cfg_.ray_enable, dyn_cfg_.ray_enable);
    pnh_.param("dyn_ray_margin_near_m", dyn_cfg_.ray_margin_near_m,
               dyn_cfg_.ray_margin_near_m);
    pnh_.param("dyn_ray_max_range_m", dyn_cfg_.ray_max_range_m,
               dyn_cfg_.ray_max_range_m);
    pnh_.param("dyn_ray_hit_eps_m", dyn_cfg_.ray_hit_eps_m,
               dyn_cfg_.ray_hit_eps_m);
    pnh_.param("dyn_ray_max_steps", dyn_cfg_.ray_max_steps,
               dyn_cfg_.ray_max_steps);
    pnh_.param("dyn_ray_confirm_df_m", dyn_cfg_.ray_confirm_df_m,
               dyn_cfg_.ray_confirm_df_m);
    pnh_.param("dyn_ray_step_m", dyn_cfg_.ray_step_m, dyn_cfg_.ray_step_m);
    pnh_.param("dyn_front_protect_enable", dyn_cfg_.front_protect_enable,
               dyn_cfg_.front_protect_enable);
    pnh_.param("dyn_front_protect_x_min_m", dyn_cfg_.front_protect_x_min_m,
               dyn_cfg_.front_protect_x_min_m);
    pnh_.param("dyn_front_protect_x_max_m", dyn_cfg_.front_protect_x_max_m,
               dyn_cfg_.front_protect_x_max_m);
    ...
    dyn_cfg_.keep_ratio = Clamp(dyn_cfg_.keep_ratio, 0.05, 1.0);
    ...
    if (dyn_cfg_.ray_max_range_m <= 0.0) {
      dyn_cfg_.ray_max_range_m =
          (scan_max_range_ > 0.0)
              ? scan_max_range_
              : ((laser_max_range_ > 0.0) ? laser_max_range_ : 35.0);
    } else {
    ...
    dyn_cfg_.front_protect_max_drop_ratio =
        Clamp(dyn_cfg_.front_protect_max_drop_ratio, 0.0, 1.0);
    dyn_cfg_.front_protect_min_in_points =
  ...
  void loadMapOrThrow() {
    ...
    if (dyn_cfg_.enable) {
      const PointCloudT::Ptr map_cloud = map_.makeOccupiedPointCloud();
      if (!map_cloud || map_cloud->empty() || !dyn_filter_.setMap(map_cloud)) {
        ROS_WARN_STREAM("[AMCL] dyn_filter setMap failed -> disable dyn_filter");
  ...
  void initRos() {
    ...
    if (viz_publish_scan_cloud_) {
      scan_cloud_viz_pub_ =
          nh_.advertise<sensor_msgs::PointCloud2>(viz_scan_cloud_topic_, 2, false);
    }
    if (publish_map_) {
  ...
  void logConfig() const {
    ...
    oss << "\n================ AMCL Localizer ================\n"
        ...
        << " scan_cloud=" << viz_scan_cloud_topic_ << "\n"
        ...
        << "viz     : scan_cloud=" << (viz_publish_scan_cloud_ ? 1 : 0)
        << " topic=" << viz_scan_cloud_topic_ << "\n"
  ...
  void processScan_(const sensor_msgs::LaserScanConstPtr &scan_msg) {
    ...
    if (do_sensor_update) {
      if (!updateFilter_(scan_msg, mount, odom_pose, pose)) {
  ...
  bool runManualLocalRelocIfRequested_(
    ...
    double chosen_combined = manualRelocCombinedScore_(coarse, 0.10);
    if ((fine_opt.xy_range_m + 1e-6) < coarse_opt.xy_range_m ||
      ...
      if (fine.ok) {
        const double fine_combined = manualRelocCombinedScore_(fine, 0.05);
    ...
    if ((final_opt.xy_range_m + 1e-6) < chosen_xy_range_m ||
      ...
      if (final.ok) {
        const double final_combined = manualRelocCombinedScore_(final, 0.01);
  ...
  bool getDynFilterPoseGuess_(const Pose2D &odom_pose,
                              Pose2D *map_pose_guess) const {
    if (!map_pose_guess) {
      return false;
    }
    if (loc_state_ != AmclLocState::GOOD) {
    ...
    if (init_validation_pending_ || frontend_mode_ != FrontendMode::NORMAL ||
        localRelocSettlingPending_() || !has_last_good_map_to_odom_) {
      return false;
    }

    *map_pose_guess = ComposePose(last_good_map_to_odom_, odom_pose);
    return true;
  }

  bool updateFilter_(const sensor_msgs::LaserScanConstPtr &scan_msg,
                     const LaserMount &mount, const Pose2D &odom_pose,
    ...
    Pose2D dyn_pose_guess;
    const bool dyn_ready = dyn_cfg_.enable && dyn_filter_.isReady() &&
                           getDynFilterPoseGuess_(odom_pose, &dyn_pose_guess);
    PointCloudT::Ptr dyn_cloud(new PointCloudT);
    std::vector<uint8_t> dyn_candidate_mask(
        static_cast<std::size_t>(ldata.range_count), 0);
    std::vector<uint8_t> dyn_keep_mask(
        static_cast<std::size_t>(ldata.range_count), 0);
    if (dyn_ready) {
      dyn_cloud->reserve(static_cast<std::size_t>(ldata.range_count));
    }

    for (int i = 0; i < ldata.range_count; ++i) {
      const double r =
          static_cast<double>(scan_msg->ranges[static_cast<std::size_t>(i)]);
      const double ang_laser =
          static_cast<double>(scan_msg->angle_min) +
          static_cast<double>(i) * static_cast<double>(scan_msg->angle_increment);
      if (r <= range_min) {
      ...
      if (!dyn_ready || !std::isfinite(r) || r <= range_min || r >= ldata.range_max) {
        continue;
      }

      const double lx = r * std::cos(ang_laser);
      const double ly = r * std::sin(ang_laser);
      const double bx = c * lx - s * ly + mount.x;
      const double by = s * lx + c * ly + mount.y;
      ...
      PointT pt;
      pt.x = static_cast<float>(bx);
      pt.y = static_cast<float>(by);
      pt.z = 0.0f;
      pt.intensity = static_cast<float>(i);
      dyn_cloud->push_back(pt);
      dyn_candidate_mask[static_cast<std::size_t>(i)] = 1;
    }

    if (dyn_ready && !dyn_cloud->empty()) {
      DynamicFilterStats dyn_stats{};
      const Eigen::Vector2f sensor_origin_base(static_cast<float>(mount.x),
                                               static_cast<float>(mount.y));
      const PointCloudT::Ptr filtered =
          dyn_filter_.filter(dyn_cloud, Pose2DToMatrix4f(dyn_pose_guess),
                             sensor_origin_base, &dyn_stats);
      const bool dyn_use_raw =
          (!filtered || filtered->empty() || dyn_stats.insufficient_static ||
           dyn_stats.kept >= dyn_stats.in || dyn_stats.front_recovered);

      if (!dyn_use_raw) {
        for (const auto &pt : filtered->points) {
          const int beam_index = static_cast<int>(std::lround(pt.intensity));
          if (beam_index < 0 || beam_index >= ldata.range_count) {
            continue;
          }
          dyn_keep_mask[static_cast<std::size_t>(beam_index)] = 1;
        }

        for (int i = 0; i < ldata.range_count; ++i) {
          const std::size_t idx = static_cast<std::size_t>(i);
          if (dyn_candidate_mask[idx] && !dyn_keep_mask[idx]) {
            ldata.ranges[i][0] = ldata.range_max;
          }
        }
      }

      if (dyn_cfg_.front_protect_enable && dyn_stats.in > 200 &&
          dyn_stats.front_in == 0) {
        ROS_WARN_STREAM_THROTTLE(
            2.0, "[AMCL] event=dyn_front_empty in=" << dyn_stats.in
                     << " front=0 axis="
      ...
      ROS_INFO_STREAM_THROTTLE(
          1.0, "[AMCL] dyn_sensor used=1 keep="
                   << dyn_stats.kept << "/" << dyn_stats.in
                   << " ratio=" << FmtD(dyn_stats.keep_ratio, 2)
                   << " p50=" << FmtD(dyn_stats.dist_p50, 2)
                   << " p90=" << FmtD(dyn_stats.dist_p90, 2)
                   << " ray=" << dyn_stats.ray_rejected << "/"
                   << dyn_stats.ray_tested
                   << " rmap50=" << FmtD(dyn_stats.rmap_p50, 2)
                   << " rmap90=" << FmtD(dyn_stats.rmap_p90, 2)
                   << " front=" << dyn_stats.front_kept << "/"
                   << dyn_stats.front_in
                   << " fdrop=" << FmtD(dyn_stats.front_drop_ratio, 2)
                   << " frec=" << (dyn_stats.front_recovered ? 1 : 0)
                   << " use_raw=" << (dyn_use_raw ? 1 : 0)
                   << " insuff=" << (dyn_stats.insufficient_static ? 1 : 0));
    }

    const ros::Time stamp =
        scan_msg->header.stamp.isZero() ? ros::Time::now() : scan_msg->header.stamp;
    publishProcessedScanCloud_(ldata, mount, stamp);
  ...
  void publishProcessedScanCloud_(const nav2_amcl::LaserData &ldata,
                                  const LaserMount &mount,
                                  const ros::Time &stamp) {
    if (!viz_publish_scan_cloud_ || !scan_cloud_viz_pub_) {
      return;
    }

    PointCloudT cloud_base;
    cloud_base.reserve(static_cast<std::size_t>(std::max(0, ldata.range_count)));
    for (int i = 0; i < ldata.range_count; ++i) {
      const double r = ldata.ranges[i][0];
      const double bearing = ldata.ranges[i][1];
      if (!std::isfinite(r) || r <= 0.0 || r >= ldata.range_max) {
        continue;
      }

      PointT pt;
      pt.x = static_cast<float>(mount.x + r * std::cos(bearing));
      pt.y = static_cast<float>(mount.y + r * std::sin(bearing));
      pt.z = 0.0f;
      pt.intensity = static_cast<float>(r);
      if (!std::isfinite(pt.x) || !std::isfinite(pt.y)) {
        continue;
      }
      cloud_base.push_back(pt);
    }

    cloud_base.width = static_cast<std::uint32_t>(cloud_base.size());
    cloud_base.height = 1;
    ...
    scan_cloud_viz_pub_.publish(msg);
  }
  ...
  ros::Publisher scan_cloud_viz_pub_;
  ...
  std::string viz_scan_cloud_topic_ = "amcl_scan_cloud";

  bool publish_tf_ = true;
  bool publish_map_ = true;
  bool publish_particlecloud_ = true;
  bool viz_publish_scan_cloud_ = true;
