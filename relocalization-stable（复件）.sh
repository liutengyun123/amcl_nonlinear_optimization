...
if [[ "${MAP_TYPE}" == "pcd" ]]; then
  ...
  # pgm/yaml 模式最终都走 map_server，因此必须有 yaml
  if [[ -z "${YAML_PATH}" ]]; then
  ...
  if ! rospack find map_server >/dev/null 2>&1; then
    echo "[ERROR] 未找到 ROS 包 map_server，请先安装，例如："
    echo "        sudo apt-get install ros-noetic-map-server"
    exit 1
  fi
fi
...
if [[ "${MAP_TYPE}" == "pcd" ]]; then
  ...
  echo "[INFO] 使用 map_server 发布 /map"

  rosrun map_server map_server "${YAML_PATH}" _frame_id:="${MAP_FRAME_ID}" &
  MAP_PUBLISH_PID=$!

  sleep 0.5
  if ! ps -p "${MAP_PUBLISH_PID}" >/dev/null 2>&1; then
    echo "[ERROR] map_server 启动失败"
    exit 1
  fi
fi
...
if [[ "${MAP_TYPE}" == "pcd" ]]; then
  ...
  echo "[INFO] RViz 中请查看 Map 话题: /map"
fi
